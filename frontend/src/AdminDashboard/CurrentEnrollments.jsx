...
export default function CurrentEnrollments() {
    ...
    // Initialize with current time and one hour later
    const now = new Date();
    const oneHourLater = new Date(now.getTime() + 60 * 60 * 1000);
    const [startTime, setStartTime] = useState(now.toTimeString().slice(0, 5)); // HH:MM format
    const [endTime, setEndTime] = useState(oneHourLater.toTimeString().slice(0, 5)); // HH:MM format
    ...
    const getEnrollments = () => {
        axios.get(`/api/v1/enrollment/time?startTime=${startTime}&endTime=${endTime}`).then(res => setEnrollments(res.data.enrollments)).catch(err => console.log(err))
    }
